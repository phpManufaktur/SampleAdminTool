...
class sample_admintool {
	...
	protected function getRecord() {
        ...
        if ($query->numRows() < 1) {
            ...
            $this->setFieldSubject(''); // set empty string
    ...
    protected function setRecord($record = array()) {
        ...
        if (isset($record['subject'])) {
            ...
            $this->setFieldSubject($record['subject']);
            $changes++;
        }
        ...
        if ($this->getFieldID() < 1) {
            ...
            $SQL =  "INSERT INTO ".TABLE_PREFIX."mod_sample_admintool (sample_id, sample_subject, sample_text) ".
                    "VALUES (1, '".$this->getFieldSubject()."', '".$this->getFieldText()."')";
            ...
            $SQL =  "UPDATE ".TABLE_PREFIX."mod_sample_admintool SET ".
                    "sample_subject='".$this->getFieldSubject()."', ".
                    "sample_text='".$this->getFieldText()."' WHERE sample_id='1'";
            // exec the SQL statement
            if (!$database->query($SQL)) {
                // on error save the database prompt and return false
                $this->setError(sprintf('[%s - %s] %s', __METHOD__, __LINE__, $database->get_error()));
